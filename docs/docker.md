...
如果你在 Apple Silicon 或多架构环境构建镜像，可能遇到 `exec format error`。这是镜像架构与宿主机不匹配导致。可在构建或运行时显式指定平台：

```bash
docker build --platform linux/amd64 -t app:amd64 .
docker run --platform linux/amd64 app:amd64
```

综合来看，Docker 真正提升的是工程确定性。你把“环境依赖、启动命令、配置约束、网络关系”全部写进可版本化文本，团队协作就不再依赖口口相传。只要仓库在，构建链路在，环境就可重复，这正是现代交付体系最需要的能力。
