...
Linux 排障里最容易被忽略的是“时间线一致性”。很多人看到报错就立刻修配置，但没有先确认错误出现的时间点，结果把因果关系搞反。更可靠的做法是先建立时间线：服务何时开始异常、当时是否有发布或配置变更、同一时段系统资源是否突增。`journalctl` 的 `--since` 与 `--until` 在这类场景非常关键。

```bash
journalctl -u nginx --since \"2026-03-13 10:00:00\" --until \"2026-03-13 10:10:00\" --no-pager
```

拿到日志窗口后，再结合 `top`、`ss`、`df` 判断是应用错误、资源瓶颈还是网络阻塞。这个方法的好处是可复盘：你不仅解决了当下问题，也能把排障过程沉淀为 SOP，后续团队成员可以直接复用。
