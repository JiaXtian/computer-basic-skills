...
SSH 经常被理解成“远程登录命令”，但它的价值是“在不可信网络中建立可信通道”。你输入 `ssh user@host` 的那一刻，背后是完整链路：客户端先验证服务器身份，协商加密算法，再用密钥证明“我是合法用户”，最后才开始传输命令与数据。理解这条链路后，遇到 `Permission denied (publickey)` 或 `Host key verification failed` 会更冷静。
...
SSH 的关键不在命令数量，而在使用习惯。建议把“高频连接”和“高风险连接”分开管理：高频连接通过 `~/.ssh/config` 固定 Host、端口和密钥，降低手误；高风险连接（生产主机、跳板机）使用独立密钥并缩短轮换周期。

另一个常见误区是把连通性问题和认证问题混在一起排查。实际上它们是两条路径：连通性看网络和端口，认证看密钥与权限。你可以先 `nc -zv server.example.com 22` 判断网络，再用 `ssh -vvv ops@server.example.com` 看认证细节。把问题分层后，排查会从“撞运气”变成“可复现”。
