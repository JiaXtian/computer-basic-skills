...
团队维度上，Git 最容易被忽略的是“约定”。建议至少统一三件事：提交消息规范、分支命名规范、PR 最小信息模板。接下来应持续强化：每个 PR 描述要包含变更目的、验证方式、风险与回滚方案。技术协作质量，往往决定于这些看似“非代码”的细节。
...
冲突并不等于错误，它只是 Git 告诉你“同一逻辑在两条历史线上被同时修改”。处理冲突时最忌讳“为了快先全部接受一边”，因为文本合并成功并不代表语义正确。更稳妥的顺序是：先看冲突文件在两个分支分别解决了什么问题，再判断是否可以并存；如果不能并存，回到需求语义决定最终行为。你可以用 `git log -- path/to/file` 查看该文件最近的修改意图，减少盲修。

对于复杂冲突，建议在本地先通过最小回归验证再继续 `rebase` 或 `merge`。例如后端项目至少跑一次关键单测，脚本项目至少执行一次关键命令链路。冲突处理的本质不是“把标记删掉”，而是“保证行为仍然正确”。当团队把这个标准统一后，冲突将不再是协作阻力，而是质量防线。
