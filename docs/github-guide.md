...
GitHub 的核心协作单元不是“直接推 main”，而是“分支 + Pull Request”。推荐流程：从 `main` 拉出 `feature/*` 分支，在分支完成开发并推送，然后发起 PR。PR 页面不仅用于合并代码，还用于解释变更目的、风险、验证方式。高质量 PR 描述通常包含：做了什么、为什么做、怎么验证、可能影响、如何回滚。你现在项目里的 PR 模板就是很好的规范基础。
...
GitHub Actions 是自动化中枢。你可以把“构建、测试、部署”写成 YAML 工作流，触发条件可以是 push、pull_request、定时任务或手动触发。你当前项目已经包含 CI 与 Pages 发布流水线，这正是典型实践：PR 阶段做质量门禁，main 合并后自动发布。对新手来说，最重要的不是记住所有语法，而是先理解三件事：触发时机（on）、执行环境（runs-on）、执行步骤（steps）。
...
GitHub Pages 适合文档站、项目主页和教程站发布。当前项目采用 Actions 构建后发布到 Pages，这是比“手工上传静态文件”更稳定的做法。配置关键点包括：`Settings -> Pages` 选择 GitHub Actions、Workflow 里正确上传 `site` 产物、main 分支触发部署。部署后通常几分钟可访问，如果页面空白，优先检查 workflow 日志与 `site_url` 配置。
